...
In this project, we build a Deep Learning based Image Search Engine that will help customers find similar products to the ones they want!
...
Our client had been analyzing their customer feedback and faced one particular issue several times.

Their customers are aware of their great range of competitively priced clothing products - but have said they are struggling to find the products they are looking for on the website.

They are often buying much more expensive products, and then later finding out that we stocked a very similar, but lower-priced alternative.
...
Here we implement the pre-trained VGG16 network. Instead of the final MaxPooling layer, we add a **Global Average Pooling Layer** at the end of the VGG16 architecture meaning the output of the network will be a single vector of numeric information rather than many arrays.  We use "feature vector" to compare image similarity.
...
We test two different images and plot the search results along with the cosine similarity scores.  
...
```python
# Pass in & features base image set
...
With the base-set featurise, we can now run a search on a new image from a customer!
...
image_distances, image_indices = image_neighbours.neighbors(search_feature_vector)
...
The way we have coded this up is very much for the "proof of concept".  In practice we would have the last section of the code (where we submit a search) isolated, and running from all of the saved objects that we need - we wouldn't include it in a single script like we have here.
